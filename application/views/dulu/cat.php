...
<html lang="en">
	...
	$(document).ready(function () {
        ...
        var jam   = waktuSplit[0] - waktuSekarangSplit[0];
            ...
            function hitung() {
               ...
               $('#timer').html(
                      '<label class="control-label" '+peringatan+'>Sisa waktu anda  (' + jam + ' : ' + menit + ' : ' + detik + ')</label>'
                );
	...
	function simpanSoal($idN, $idB, $id)
	{
		...
		var url = "<?php echo site_url('cat/saveSoal')?>/"+$id+"/"+dana;
		...
		$.ajax({
			...
			success: function(data)
			{
				if(data.status){
					...
					window.location.href = '<?=base_url()?>cat/sim/'+$resN+"/"+$resB;
				}
			},
	...
	function selesaiKerja()
	{
		...
		$('#btnSave').text('Proses...'); //change button text
		$('#btnSave').attr('disabled',true); //set button enable 
		var url = "<?php echo site_url('cat/selesaiKerja')?>";
		...
		$.ajax({
			...
			{
				if(data.status){
					window.location.href = '<?=base_url()?>hasil/paket/'+data.idParentTmp;
				}
			},
			error: function (jqXHR, textStatus, errorThrown)
			{
                                    ...
                                    <form id="form" class="form-horizontal" autocomplete="off">
                                        ...
                                        <div class="form-body">
                                            ...
                                            <div class="form-group">
												...
												<?=$s->tes_detail_id;?> <?=$idSekarang?>. <?=$s->soal_pertanyaan;?>
                                                </label>
                                            </div>
											<div class="form-group">
											<div class="col-md-9">
                                                    <div class="mt-radio-list">
                                                        <label class="mt-radio">
                                                            <input type="radio" name="optionPG<?=$idSekarang?>" id="optionPG<?=$idSekarang?>" value="A" checked=""> 
															A. <?=$s->soal_jawaban_a;?>
                                                            <span></span>
                                                        </label>
                                                        <label class="mt-radio">
                                                            <input type="radio" name="optionPG<?=$idSekarang?>" id="optionPG<?=$idSekarang?>" value="B" checked=""> 
															B. <?=$s->soal_jawaban_b;?>
                                                            <span></span>
                                                        </label>
                                                        <label class="mt-radio">
                                                            <input type="radio" name="optionPG<?=$idSekarang?>" id="optionPG<?=$idSekarang?>" value="C" checked=""> 
															C. <?=$s->soal_jawaban_c;?>
                                                            <span></span>
                                                        </label>
                                                        <label class="mt-radio">
                                                            <input type="radio" name="optionPG<?=$idSekarang?>" id="optionPG<?=$idSekarang?>" value="D" checked=""> 
															D.<?=$s->soal_jawaban_d;?>
                                                            <span></span>
                                                        </label>
                                                        <label class="mt-radio">
                                                            <input type="radio" name="optionPG<?=$idSekarang?>" id="optionPG<?=$idSekarang?>" value="E" checked=""> 
															E. <?=$s->soal_jawaban_e;?>
                                                            <span></span>
                                                        </label>
                                                    </div>
                            ...
                            <div class="portlet light ">
                                ...
                                <div class="portlet-title">
                                    <div class="caption font-dark col-md-12">													
										<button type="submit" id="btnFinish" class="btn btn-info btn-circle col-md-12" onclick="selesaiKerja()">Selesai</button>
                                    </div>
                                    <div class="tools"> </div>
                                </div>
                                <div class="portlet-title">
