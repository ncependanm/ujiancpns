...
{	
	...
	function index()
    {
		...
		$this->template->load('tema', 'backend/hasil/index', $data);
    }
	...
	function paket($id)
    {
		...
		$status = '';
		if($nilaiTWK < 70 || $nilaiTIU < 75 || $nilaiTKP < 126){
			$status = 'Gagal';
		} else {
			$status = 'Lulus';
		}
		$data['status'] = $status;
		$data['nilaiTWKBatas'] = 70;
		$data['nilaiTIUBatas'] = 75;
		$data['nilaiTKPBatas'] = 126;
